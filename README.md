...
## 핵심 기능
- 5개 포지션을 **클릭 한 번으로 저장/해제** — 연결이 끊겨도 유지(서버 서명 세션 신원, 재접속에도 유지).
- **설정 편집**: 예정 시간, 티어(아이언~챌린저·상관없음), 큐(일반게임·칼바람·솔로랭크·자유랭크·5인 자유랭·기타) — **방장만** 변경 가능.
...
- 신원 위조 방지(서버 서명 토큰), 소켓당 레이트 리밋, Origin 화이트리스트 — [ARCHITECTURE.md](docs/ARCHITECTURE.md#업데이트--v05-보안-강화) 참고.
...
운영 배포 시 `.env.example`의 `SESSION_SECRET`(세션 고정), `ALLOWED_ORIGINS`(교차 출처 차단) 설정 권장.
